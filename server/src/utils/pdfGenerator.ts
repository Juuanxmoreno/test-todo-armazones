...
export async function generateOrderPDF(
  ...
  const isParcelCompany =
    orderData.shippingMethod === ShippingMethod.ParcelCompany;
  ...
  // Traducciones
  const shippingMethodLabel = translateShippingMethod(
    orderData.shippingMethod as ShippingMethod
  );
  const paymentMethodLabel = translatePaymentMethod(
  ...
  const totalGainUSDFormatted = formatCurrency(
    orderData.totalGainUSD,
    "en-US",
    "USD"
  );
  ...
  const pdfBuffer = (await page.pdf({
    ...
    footerTemplate: `
      <div style="font-size:9px; width:100%; text-align:center; color:#444; padding:6px 0; font-family: Arial, sans-serif;">
    ...
    `,
    headerTemplate: `<div></div>`, // vacío para no mostrar nada en el header
  })) as Buffer;
